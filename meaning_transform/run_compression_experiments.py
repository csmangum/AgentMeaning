...
from src.data import AgentStateDataset, AgentState
from src.metrics import SemanticMetrics, generate_t_sne_visualization, compute_feature_drift
...
class CompressionExperiment:
    ...
    def __init__(self, base_config: Config = None, output_dir: str = None, use_real_data: bool = False):
        """
        ...
        Args:
            ...
            use_real_data: Whether to use real data from simulation.db
        """
    ...
    def _save_base_config(self):
        ...
        config_dict = {
            "model": {key: value for key, value in vars(self.base_config.model).items() 
                      if not key.startswith('_')},
            "training": {key: value for key, value in vars(self.base_config.training).items() 
                        if not key.startswith('_')},
            "data": {key: value for key, value in vars(self.base_config.data).items() 
                    if not key.startswith('_')},
            "metrics": {key: value for key, value in vars(self.base_config.metrics).items() 
                      if not key.startswith('_')},
    ...
    def run_experiments(self):
        ...
        for level in self.compression_levels:
            ...
            self.results[level] = {
                ...
                'experiment_dir': training_results.get('experiment_dir', str(self.models_dir)),
    ...
    def _prepare_data(self) -> Dict[str, AgentStateDataset]:
        ...
        if self.use_real_data:
            if os.path.exists("simulation.db"):
                ...
                # Implement real data loading here if needed
                pass
            else:
                print("Warning: simulation.db not found. Using synthetic data instead.")
                dataset.generate_synthetic_data(self.base_config.data.num_states)
        else:
            print(f"Generating {self.base_config.data.num_states} synthetic agent states...")
            dataset.generate_synthetic_data(self.base_config.data.num_states)
        ...
        train_dataset = AgentStateDataset(train_states, batch_size=self.base_config.training.batch_size)
        val_dataset = AgentStateDataset(val_states, batch_size=self.base_config.training.batch_size)
    ...
    def _analyze_results(self):
        ...
        for level, result in self.results.items():
            ...
            if os.path.exists(model_path):
                ...
                print(f"Warning: Model file not found for compression level {level}: {model_path}")
            ...
            print(f"Compression level {level}: Val Loss = {metrics['val_loss'][-1]:.4f}, "
    ...
    def _evaluate_semantic_drift(self, model: MeaningVAE) -> float:
        ...
        """
        device = torch.device("cuda" if torch.cuda.is_available() and self.base_config.use_gpu else "cpu")
        model.to(device)
        model.eval()
        ...
        drift_states = self.results.get(model.compression_level, {}).get('training_results', {}).get('drift_tracking_states', [])
    ...
    def _create_visualizations(self, results_df: pd.DataFrame):
        ...
        plt.plot(results_df['compression_level'], results_df['val_loss'], 'o-', linewidth=2)
        ...
        plt.figure(figsize=(10, 6))
        plt.plot(results_df['compression_level'], results_df['semantic_drift'], 'o-', linewidth=2, color='orange')
        ...
        plt.figure(figsize=(10, 6))
        plt.plot(results_df['compression_level'], results_df['model_size_kb'], 'o-', linewidth=2, color='green')
        ...
        max_val_loss = max(results_df['val_loss']) if len(results_df['val_loss']) > 0 else 1
        max_semantic_drift = max(results_df['semantic_drift']) if len(results_df['semantic_drift']) > 0 else 1
        max_model_size = max(results_df['model_size_kb']) if len(results_df['model_size_kb']) > 0 else 1
        
        plt.plot(results_df['compression_level'], results_df['val_loss'] / max_val_loss, 'o-', linewidth=2, label='Normalized Val Loss')
        plt.plot(results_df['compression_level'], results_df['semantic_drift'] / max_semantic_drift, 'o-', linewidth=2, label='Normalized Semantic Drift')
        plt.plot(results_df['compression_level'], results_df['model_size_kb'] / max_model_size, 'o-', linewidth=2, label='Normalized Model Size')
...
"""
        ...
        results_df['drift_size_ratio'] = results_df['semantic_drift'] / results_df['model_size_kb']
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Run compression experiments for meaning-preserving transformations")
    
    parser.add_argument("--output-dir", type=str, default="results/compression_experiments",
...
def main():
    ...
    experiment = CompressionExperiment(base_config, args.output_dir, args.use_real_data)
