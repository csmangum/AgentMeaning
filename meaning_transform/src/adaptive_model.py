...
class AdaptiveEntropyBottleneck(nn.Module):
    ...
    def __init__(self, latent_dim: int, compression_level: float = 1.0):
        ...
        self.proj_up = nn.Linear(self.effective_dim, latent_dim * 2)  # mu and log_scale for reconstruction
        
        logging.info(f"Created AdaptiveEntropyBottleneck with latent_dim={latent_dim}, "
...
class FeatureGroupedVAE(nn.Module):
    ...
    def __init__(
        ...
        for name, (start_idx, end_idx, compression) in feature_groups.items():
            ...
            self.group_latent_dims[name] = (latent_start_idx, latent_start_idx + group_latent_dim)
            ...
            logging.info(f"Feature group '{name}': features [{start_idx}:{end_idx}], "
    ...
    def get_compression_rate(self) -> Dict[str, float]:
        ...
        total_effective_dim = sum(bottleneck.effective_dim for bottleneck in self.bottlenecks.values())
    ...
    def get_feature_group_analysis(self) -> Dict[str, Dict[str, Any]]:
        ...
        for name, (start_idx, end_idx, compression) in self.feature_groups.items():
            ...
            analysis[name] = {
                ...
                "importance": 1.0 / compression  # Inverse relationship with compression
            }
