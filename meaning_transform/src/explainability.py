...
class GraphVisualizer:
    ...
    def plot_knowledge_graph(
        ...
        for node in G.nodes():
            ...
            node_color = self.node_color_map.get(
                node_type, self.node_color_map["unknown"]
            )
        ...
        for u, v, data in G.edges(data=True):
            ...
            edge_color = self.edge_color_map.get(
                edge_type, self.edge_color_map["unknown"]
            )
    ...
    def create_interactive_graph(
        ...
        for node_type, nodes in node_groups.items():
            ...
            node_trace = go.Scatter3d(
                ...
                z=z,
                mode="markers",
                marker=dict(size=marker_size, color=node_color, opacity=0.8),
                text=hover_text,
                hoverinfo="text",
                name=node_type,
            )
        ...
        for relation, edges in edge_groups.items():
            ...
            # Create edge trace
            edge_color = self.edge_color_map.get(
                relation, self.edge_color_map["unknown"]
            )
...
class LatentSpaceVisualizer:
    ...
    def __init__(self, figsize: Tuple[int, int] = (10, 8), cmap: str = "viridis"):
        """
    ...
    def _create_interactive_latent_viz(
        self,
        data: np.ndarray,
        labels: Optional[List[Any]],
        title: str,
    ...
    def interpolate_latent_space(
        ...
        if decoded is not None:
            ...
            if isinstance(decoded[0], (nx.Graph, Data)):
                ...
                ax.scatter(
                    reduced[0, 0], reduced[0, 1], c="green", s=100, label="Start"
                )
    ...
    def _visualize_graph_interpolation(
        self,
...
class ModelExplainer:
    ...
    def explain_node_importance(
        ...
        visualize: bool = True,
        save_path: Optional[str] = None,
    ) -> Tuple[torch.Tensor, Optional[plt.Figure]]:
        """
        ...
        attributions = attribution_method.attribute(
            data.x,
            target=target_nodes,
            additional_forward_args=(data.edge_index, data.edge_attr),
        )

        # Create visualization if requested
        fig = None
        if visualize:
            fig = self._visualize_node_importance(
                data, attributions, target_nodes, save_path
            )

        return attributions, fig

    def _visualize_node_importance(
        self,
        data: Data,
