...
class FeatureImportanceAnalyzer:
    ...
    def __init__(
        self, feature_extractors: List[str] = None, canonical_weights: bool = True
    ):
        """
    ...
    def extract_feature_matrix(
        self, agent_states: torch.Tensor, flatten: bool = True
    ...
    def analyze_importance_for_outcome(
        self,
    ...
    def analyze_importance_for_reconstruction(
        self, original_states: torch.Tensor, reconstructed_states: torch.Tensor
    ) -> Dict[str, float]:
        """
        ...
        original_features = self.semantic_loss.extract_semantic_features(
            original_states
        )
        reconstructed_features = self.semantic_loss.extract_semantic_features(
        ...
        for feature_name in self.feature_extractors:
            ...
            if feature_name in ["has_target", "is_alive", "threatened"]:
                ...
                recon_binary = (
                    (reconstructed_features[feature_name] > 0.5).cpu().numpy()
                )
    ...
    def analyze_importance_for_behavior(
        self, agent_states: torch.Tensor, behavior_vectors: np.ndarray
    ) -> Dict[str, float]:
        """
    ...
    def compute_importance_weights(
        self, feature_importance: Dict[str, float] = None
    ) -> Dict[str, float]:
        """
    ...
    def visualize_feature_importance(
        ...
        feature_groups = [
            self.feature_to_group.get(feature, "other") for feature in features
        ]
    ...
    def visualize_group_importance(
        self,
        ...
        legend_labels = [
            f"{group}: {feature_examples.get(group, '')}" for group in groups
        ]
        ax.legend(
